package org.stockmaketprediction.neuralnetwork;
...
public class NeuralNetworkStockPredictor {
	...
	private String learningDataFilePath = "data/input/learningData.csv";
	private String neuralNetworkModelFilePath = "stockPredictor.nnet";

	// public static void main(String[] args) throws IOException {

	public double neuralNetworkStockMarketPredictor(int slidingWindowSizeIncoming, String rawDataFilePathIncoming)
			throws Exception {

		System.out.println();
		rawDataFilePath = rawDataFilePathIncoming;
		slidingWindowSize = slidingWindowSizeIncoming;

		double[] resArr = new double[5];
		double result = 0.0;

		System.out.println("@@@@@@   ********* RAW FILE DATAPATH " + rawDataFilePathIncoming);

		// NeuralNetworkStockPredictor predictor = new
		// NeuralNetworkStockPredictor(5, "data/input/EOD-INTU-training.csv");
		...
		for (int i = 0; i < resArr.length; i++) {
			...
			resArr[i] = testNetwork();
		}
		double sum = 0.0;
		for (int i = 0; i < resArr.length; i++) {
			sum += resArr[i];
			System.out.println("@@@ iteration  " + i + "  -> " + resArr[i]);
		}
		result = sum / resArr.length;

		return result;
	}

	// public NeuralNetworkStockPredictor(int slidingWindowSize, String
	// rawDataFilePath) {
